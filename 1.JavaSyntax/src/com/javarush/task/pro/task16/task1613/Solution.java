...
//            System.out.println(dateTime + ", is weekend - " + isWeekend(dateTime));
            System.out.println(dateTime + ", is weekend - " + isWeekend(dateTime) + " ".repeat(10) + dateTime.getDayOfWeek());
        }
    ...
    {
        if ((dateTime.getDayOfWeek().getValue() == FRIDAY && dateTime.toLocalTime().isAfter(LocalTime.of(WEEKEND_START_FRIDAY_CUT_OFF_HOUR, 0).minusNanos(1)))
                || (dateTime.getDayOfWeek().getValue() == SATURDAY)
                || (dateTime.getDayOfWeek().getValue() == SUNDAY && dateTime.toLocalTime().isBefore(LocalTime.of(WEEKEND_END_SUNDAY_CUT_OFF_HOUR, 0))))
        {
            return true;
        }
        return false;
